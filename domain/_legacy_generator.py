...
# NOTE:
# Legacy implementation expects `copy_distribution.*`.
# In the refactored structure we use domain.distribution.
from . import distribution as copy_distribution
...
# ---- Master data loading (with safe fallback) -----------------------------
# NOTE:
# - For reproducibility and portability, we try to start the app even when
#   master CSVs are missing. In that case we fall back to Faker-based outputs.
# - We log warnings in both Japanese and English for debugging.
def _warn_missing_master(file_path: str, usage: str) -> None:
    msg_ja = f"警告: 必須マスタCSVが見つかりません: {file_path}（用途: {usage}）。Fakerベースのフォールバックで継続します。"
    msg_en = f"WARNING: Missing required master CSV: {file_path} (usage: {usage}). Falling back to Faker-based generation."
    print(msg_ja)
    print(msg_en)
...
last_df = None
try:
    first_df = pd.read_csv(f"{data_path}first_name_sorted.csv")
except FileNotFoundError:
    _warn_missing_master(f"{data_path}first_name_sorted.csv", "氏名/かな/性別/ローマ字")
try:
    last_df = pd.read_csv(f"{data_path}last_name.csv")
except FileNotFoundError:
    _warn_missing_master(f"{data_path}last_name.csv", "氏名/かな/ローマ字")

# 県データ
ken_df = None
ken_path = f"{data_path}KEN_ALL2.csv"
try:
    ken_df = pd.read_csv(ken_path)
except FileNotFoundError:
    _warn_missing_master(ken_path, "郵便番号/住所")

# 会社名
comp_df = None
compony_path = f"{data_path}compony_data.csv"
try:
    comp_df = pd.read_csv(compony_path)
except FileNotFoundError:
    _warn_missing_master(compony_path, "会社名")
...
# 郵便番号と住所
def zip_address():
    # Fallback: no postal master → use Faker
    if ken_df is None:
        zip_code = fac.postcode()
        address = fac.address().replace("\n", "")
        return zip_code, address[:25]
    is_ok = False
    while not is_ok:
...
def zip_address_area_ja():
    ...
    if ken_df is None:
        zip_code = fac.postcode()
        area_code = str(zip_code).replace("-", "")[:3]
        address = fac.address().replace("\n", "")
        return zip_code, address[:25], area_code
    is_ok = False
    while not is_ok:
        ken_n = random.randrange(len(ken_df))
        zip_tmp = str(ken_df["zip_code"][ken_n])
...
# 会社名
def compony():
    if comp_df is None or "会社名" not in comp_df.columns:
        return fac.company()
...
def sex_to_num(sex):
    # Fallback: no name master → just return 0
    if first_df is None:
        return 0
    if sex == "その他・不明":
...
def name_kana_sex_email_from_sex(sex):
    if first_df is None or last_df is None:
        # Without name master, we cannot map sex→row index reliably.
        return name_kana_sex_email_random()
...
def make_image_and_save(x_list,y_list,jp_title,figure_path,en_name):
    # Matplotlib fallback (avoid Plotly/Kaleido dependency at runtime)
    fig, ax = plt.subplots(figsize=(6, 4))
    ax.bar(x_list, y_list)
    ax.set_title(jp_title)
    ax.tick_params(axis="x", rotation=0)
    out_path = os.path.join(figure_path, en_name)
    fig.tight_layout()
...
def age_imagesave_and_statistics(age_list,age_end,age_start,df,original_df,figure_path):
    ...
    ax.set_title("年齢分布")
    fig.tight_layout()
    fig.savefig(os.path.join(figure_path, "age.png"), dpi=150)
    plt.close(fig)
    # 分布類似度
    x_list = age_list        
